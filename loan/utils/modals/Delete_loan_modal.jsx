...
const Delete_loan_modal = ({ open, close }) => {

    const { handle_delete_myloan, loan_id } = useStateContext();

    const handle_delete = () => {
        handle_delete_myloan(loan_id);
        close();
    };

    return (
        <Dialog
            open={open.delete_loan_modal}
            onClose={close}
        >
            <div className=' md:w-[500px] p-6 md:p-7 relative flex flex-col gap-8 md:gap-10' >
                ...
                <p className='text-[13px] md:text-[16px] text-stone-600 font-medium' >Are you sure, do you want to delete this Loan?</p>
